...
export default function AdminConsole() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'verifications' | 'ads' | 'diagnostics'>('overview');
  ...
  const loadAdminData = async () => {
    setIsLoading(true);
    try {
      const [s, companies, ads, audit] = await Promise.all([
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
            <div>
              ...
              <h1 id="admin-title" className="text-5xl font-black text-primary leading-tight">
                Consola de <span className="text-accent">Control Maestro</span>
              </h1>
              <p className="text-muted-foreground font-bold italic mt-2">Gestión de monetización, auditoría y autorizaciones AGM</p>
            </div>
            
            <div className="flex space-x-2 bg-white p-2 rounded-[2rem] border-2 border-primary/5 shadow-xl scrollbar-hide overflow-x-auto max-w-full">
              {(['overview', 'verifications', 'ads', 'diagnostics'] as const).map((tab) => (
                <button
                  key={tab}
                  id={`tab-${tab}`}
                ...
                >
                  {tab === 'overview' ? 'Resumen' : tab === 'verifications' ? 'Verificaciones' : tab === 'ads' ? 'Anuncios' : 'Diagnósticos'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-[4rem] border-2 border-primary/5 shadow-2xl overflow-hidden min-h-[600px] animate-fade-in-up">
            ...
            {activeTab === 'overview' && (
              <div className="p-10 flex flex-col items-center justify-center min-h-[500px]">
                ...
                <h3 className="text-3xl font-black text-primary mb-4">Ecosistema B2B AGM Activo</h3>
